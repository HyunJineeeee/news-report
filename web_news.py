...
def summarize_with_gemini(text: str) -> str:
    ...
    try:
        ...
        prompt = (
            "너는 뉴스 요약 전문가야. 아래 뉴스 기사를 읽고 핵심 내용을 3줄 이내로 요약해줘.\n"
            "조건 1: '- '로 시작하는 개조식 문장.\n"
            "조건 2: 구체적인 수치나 사실 관계 위주로.\n\n"
            f"기사 내용:\n{text[:4000]}"
        )
...
def crawl_naver_news(keyword, target_date_str):
    """
    네이버 API를 통해 뉴스 검색 (하루치)
    """
    if not NAVER_CLIENT_ID or not NAVER_CLIENT_SECRET:
        print("[ERROR] 네이버 API 키가 없습니다.")
        return []
    ...
    for item in data.get('items', []):
        # 날짜 파싱 (Format: "Mon, 01 Dec 2025 14:00:00 +0900")
        try:
            pub_date_dt = datetime.strptime(item['pubDate'], "%a, %d %b %Y %H:%M:%S +0900")
            pub_date_str = pub_date_dt.strftime("%Y-%m-%d %H:%M") # KST String
            pub_date_day = pub_date_dt.strftime("%Y-%m-%d")
        except:
            continue

        # 타겟 날짜(어제)가 아니면 스킵 (API는 최신순이므로 날짜 지나면 break 가능하지만 안전하게 다 확인)
        if pub_date_day != target_date_str:
            continue
            
        # originallink가 있으면 그걸 쓰고, 없으면 link(네이버뉴스) 사용
        ...
        rows.append({
            ...
            "제목": title,
            "원문링크": final_link,
            "출처": "NaverSearch", # API는 출처를 따로 안 줌 (도메인 파싱 필요시 추가)
            "발행일(KST)": pub_date_str,
            "수집시각(KST)": collected_at,
            "요약": "",
            "_link_for_check": final_link # 중복 제거용
        })
...
def send_email_report(df_new, target_date_str):
    ...
    <div style="font-family: 'Malgun Gothic', sans-serif; background-color: #f4f4f4; padding: 20px; color: #333;">
        <div style="max-width: 700px; margin: 0 auto; background-color: #ffffff; padding: 30px; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
            <div style="text-align: center; margin-bottom: 30px; border-bottom: 2px solid #27ae60; padding-bottom: 20px;">
    ...
    for kw in KEYWORDS:
        if kw in grouped.groups:
            ...
            <div style="margin-bottom: 30px;">
                <div style="background-color: #27ae60; color: white; padding: 6px 15px; display: inline-block; border-radius: 15px; font-weight: bold; font-size: 16px; margin-bottom: 15px;">
                    # {kw}
                </div>
            """
            for idx, row in group_df.iterrows():
                ...
                if summary:
                    ...
                    summary_style = "background-color: #f9f9f9; padding: 15px; border-left: 4px solid #27ae60; color: #555; font-size: 14px; line-height: 1.6; border-radius: 4px;"
                else:
    ...
    html_body += """
            <div style="text-align: center; margin-top: 40px; font-size: 12px; color: #bdc3c7; border-top: 1px solid #eee; padding-top: 20px;">
                Automated by GitHub Actions & Naver Search API
            </div>
        </div>
    </div>
    """
...
def main():
    ...
    req_cols = ["키워드","제목","원문링크","발행일(KST)","수집시각(KST)","출처","요약","_link_for_check"]
    ...
    if all_path.exists():
        ...
        existing_links = set(df_existing["_link_for_check"].dropna().astype(str))
    else:
    ...
    # 중복 제거
    df_crawled["_is_new"] = ~df_crawled["_link_for_check"].astype(str).isin(existing_links)
    df_crawled = df_crawled.drop_duplicates(subset=["_link_for_check"], keep="first")
    ...
    for idx, row in df_to_process.iterrows():
        ...
        if content:
            # 1. 필터링: 내용에 키워드 확인 (정확도 UP)
            if keyword not in content and keyword not in row['제목']:
                print(f"   ❌ [제외] 본문/제목에 '{keyword}' 없음")
                continue 
            ...
            # 본문 추출 실패 시 (네이버 뉴스 라이브러리 등이 아닌 경우)
            # 그래도 API가 준 제목이 정확하므로 포함은 시킴
            pass
    ...
    # _link_for_check 기준 중복제거
    combined = combined.drop_duplicates(subset=["_link_for_check"], keep="last")
    # 정렬 (날짜 기준)
